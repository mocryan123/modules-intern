 ...
 * File: paymaya.php
 * Include this in main.php via: require_once(plugin_dir_path(__FILE__) . 'paymaya.php');
 *
 * Covers:
...
// ─────────────────────────────────────────────
// CONFIG — PayMaya API Keys (from .env)
// https://dashboard.paymaya.com → Developers → API Keys
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// AJAX: Create PayMaya Checkout Session
...
function bntm_ajax_bae_pm_checkout() {
    ...
    if ($code !== 200 || empty($body['data']['attributes']['checkout_url'])) {
        $err = $body['errors'][0]['detail'] ?? 'PayMaya error. Please try again.';
...
// Runs when user returns from PayMaya checkout
...
// ─────────────────────────────────────────────
// WEBHOOK LISTENER
// Register this URL in PayMaya Dashboard:
// Dashboard → Developers → Webhooks → Add Endpoint
// URL: https://yoursite.com/?bae_webhook=paymaya
...
// ─────────────────────────────────────────────
// HELPER: Make PayMaya API request
// ─────────────────────────────────────────────
function bae_pm_request($method, $endpoint, $body = null) {
    ...
    return wp_remote_request('https://pg.paymaya.com' . $endpoint, $args);
}
...
function bae_pm_verify_signature($payload, $sig_header) {
    ...
    // PayMaya signature format: t=timestamp,te=hash,li=hash
    $parts = [];
