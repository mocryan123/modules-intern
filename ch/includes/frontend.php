...
function ch_render_avatar($display_name, $avatar_url = '', $wrapper_class = 'ch-avatar-sm', $img_class = '', $attrs = '') {
    ...
    <div class="<?php echo esc_attr($wrapper_class); ?>" <?php echo $attrs; ?>>
        <?php if ($avatar_url): ?>
        <img src="<?php echo esc_url($avatar_url); ?>" alt="<?php echo esc_attr($display_name); ?>" class="<?php echo esc_attr($img_class_attr); ?>">
        <?php else: ?>
...
function ch_render_category_options($categories) {
    ...
    foreach ($categories as $cat): ?>
        <option value="<?php echo (int)$cat->id; ?>" data-private="<?php echo !empty($cat->is_private) ? '1' : '0'; ?>"><?php echo esc_html($cat->name); ?></option>
    <?php endforeach;
    return ob_get_clean();
}
    ...
    <div id="<?php echo esc_attr($args['modal_id']); ?>" class="ch-modal-overlay" style="display:none">
        <div class="ch-modal ch-modal-lg ch-composer-modal">
            <div class="ch-composer-header">
                ...
                <button type="button" class="ch-composer-close" onclick="chCloseModal('<?php echo esc_js($args['modal_id']); ?>')" aria-label="Close">&times;</button>
            </div>
            ...
            <div class="ch-composer-body">
                ...
                <input type="text" id="<?php echo esc_attr($args['guest_name_id']); ?>" class="ch-composer-guest-name" placeholder="Your name (leave blank to post anonymously)" maxlength="100">
                <?php endif; ?>

                <input type="text" id="<?php echo esc_attr($args['title_id']); ?>" class="ch-composer-title" placeholder="<?php echo esc_attr($args['title_placeholder']); ?>">
                <textarea id="<?php echo esc_attr($args['content_id']); ?>" class="ch-composer-textarea" rows="4" placeholder="<?php echo esc_attr($args['content_placeholder']); ?>"></textarea>
                ...
                <div class="ch-composer-tags-row">
                    ...
                    <input type="text" id="<?php echo esc_attr($args['tags_id']); ?>" class="ch-composer-tags-input" placeholder="Tags: road, drainage, urgent...">
                </div>

                <label class="ch-composer-media-area" id="<?php echo esc_attr($args['media_label_id']); ?>" for="<?php echo esc_attr($args['media_input_id']); ?>">
                    ...
                    <input type="file" id="<?php echo esc_attr($args['media_input_id']); ?>" style="display:none" multiple accept="image/*,video/*,audio/*"
            ...
            <div class="ch-composer-footer">
                ...
                <button type="button" class="ch-btn ch-btn-primary ch-composer-submit" id="<?php echo esc_attr($is_edit ? 'ch-update-post-btn' : 'ch-submit-post-btn'); ?>"
...
function ch_media_kind_from_url($url) {
    ...
    if (in_array($ext, ['mp4', 'webm', 'ogg', 'mov', 'm4v'], true)) return 'video';
...
function ch_guest_landing_page() {
    ...
    <div class="ch-guest-landing-wrap">
        <aside class="ch-guest-sidebar">
            <!-- Mobile Drawer Close -->
            <button class="ch-drawer-close" onclick="document.body.classList.remove('ch-drawer-open')" aria-label="Close Menu" style="display:none;position:absolute;top:16px;right:16px;background:none;border:none;font-size:24px;cursor:pointer;color:var(--ch-text-muted);">&times;</button>
                    ...
                    <div class="ch-setting-row">
                        ...
                        <div class="ch-setting-control">
                            <input type="number" name="ch_media_upload_limit"
                                   value="<?php echo esc_attr(get_option('ch_media_upload_limit', 6)); ?>"
                                   min="1" max="20" class="ch-input ch-setting-number">
    ...
    if ($tab === 'profile') {
        ...
        <div class="ch-profile-standalone">
            <nav class="ch-top-nav">
                ...
                <div class="ch-user-bar">
                    <button class="ch-avatar-btn" id="ch-profile-btn" onclick="chToggleProfileMenu(event)" aria-label="Profile" data-ch-current-user-avatar="1" data-avatar-name="<?php echo esc_attr($current_display_p); ?>">
                    ...
                    <div class="ch-dropdown-panel ch-dropdown-panel-sm" id="ch-profile-menu" style="display:none;">
                        <div class="ch-dropdown-user-info">
                            <div class="ch-avatar-btn ch-avatar-btn-lg" data-ch-current-user-avatar="1" data-avatar-name="<?php echo esc_attr($current_display_p); ?>"><?php echo ch_render_avatar($current_display_p, $current_avatar_p, 'ch-avatar-btn-inner ch-avatar-btn-inner-lg', 'ch-current-user-avatar-img'); ?></div>
                            <div>
    ...
    <div class="ch-feed-wrap">
        ...
        <main class="ch-feed-main">
            ...
            <div class="ch-composer-trigger-card" onclick="<?php echo $can_post_here ? "chOpenModal('ch-modal-create-post')" : "void(0)"; ?>" role="button" tabindex="0"
                ...
                <div class="ch-composer-trigger-avatar <?php echo !$user_id ? 'ch-composer-trigger-guest' : ''; ?>" <?php echo $user_id ? 'data-ch-current-user-avatar="1" data-avatar-name="' . esc_attr($current_display_feed) . '"' : ''; ?>>
                    <?php if ($user_id): ?>
