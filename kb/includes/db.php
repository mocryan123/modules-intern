...
function bntm_kbf_get_tables() {
    ...
    return [
        'kbf_funds' => "CREATE TABLE {$prefix}kbf_funds (
            ...
            milestones TEXT COMMENT 'JSON array of milestone updates',
...
function bntm_kbf_ensure_profile_columns() {
    ...
    $cols = [
        ...
        'profile_type'        => "ALTER TABLE {$pt} ADD COLUMN profile_type VARCHAR(50) NULL",
...
function bntm_kbf_ensure_fund_columns() {
    ...
    $cols = [
        ...
        'milestones' => "ALTER TABLE {$ft} ADD COLUMN milestones TEXT NULL",
    ];
    foreach ($cols as $col => $sql) {
        if (!in_array($col, $existing, true)) {
            $wpdb->query($sql);
